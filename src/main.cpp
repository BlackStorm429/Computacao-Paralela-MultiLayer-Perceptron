...
int main(int argc, char* argv[]) {
    ...
    if (rank == 0) 
    {
        ...
        // Treino OpenMP
        cout << "\nTreinamento MLP OpenMPcom " << omp_get_max_threads() << " threads\n\n";
        ...
        cout << "\nTreinamento MLP OpenMP GPU com " << omp_get_max_threads() << " threads\n\n";

        MLP_OpenMP_GPU gpuNet(mlp_base, omp_get_max_threads());
        MLPTester gpuTester(gpuNet);
        int64_t gpu_duration = gpuTester.train(1000, 0.15, Xtrain, Ytrain);
        ...
        cout << "Duração OpenMP GPU: " << gpu_duration << " ms\n";
        cout << "Speedup GPU/OpenMP: " << static_cast<double>(openmp_duration) / gpu_duration << "x\n";
