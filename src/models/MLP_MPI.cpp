...
public:
    MLP_MPI(const int* layerSizes,
      ...
      : learningRate(learningRate),
        ...
        openMP(layerSizes, batchSize, numThreads, learningRate)
    {
    ...
    double loss(const std::vector<double>& input,
                const std::vector<double>& target) override
    {
               ...
               const std::vector<std::vector<double>>& outputData) override
    {
        ...
        for (int i = 0; i < weightSize; ++i) {
            double noise = ((double(std::rand()) / RAND_MAX - 0.5) * 2.0) 
                           * learningRate;
        ...
        MPI_Gather(localTrainedFlat.data(), weightSize, MPI_DOUBLE,
                   rank == 0 ? allTrainedWeights.data() : nullptr,
                   weightSize, MPI_DOUBLE,
                   0, MPI_COMM_WORLD);
